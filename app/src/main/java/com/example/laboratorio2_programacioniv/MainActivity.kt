...
class MainActivity : AppCompatActivity() {
    ...
    fun registrar(v: View){
        ...
        if(!nombre.isEmpty() && !edad.isEmpty() && !departamento.isEmpty()){
            ...
            val adaptador: ArrayAdapter<String> = ArrayAdapter<String>(this, R.layout.list_item, nombres)
            lvNombres.adapter = adaptador

            lvNombres.onItemClickListener = object : AdapterView.OnItemClickListener{
                override fun onItemClick(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                    tvDatos.text = "Datos de la persona: Nombre: ${lvNombres?.getItemAtPosition(position)}    Edad: ${edades[position]}años    Departamento: ${departamentos[position]} El Salvador"
                }
            }
